...
@section('content')
    ...
    @include('admin.template.modal', ['id' => 'showDetail_Modal', 'title'=>'Form Modules', 'form'=>'admin.modules.list'])
@endsection
@push('script')
    {{ $dataTable->scripts() }}
    <script>
        function deleteModules(data) {
            ...
            $.post('/admin/modules/destroy', dataPost).done(function (response) {
        ...
        function addModules(e) {
            ...
            $('#uri').val('').change();
            $('#icon').val('').change();
            window.urlMethod = '/admin/modules/store';
        }

        function detailModules(_this) {
            let dataPost = {};
            dataPost.id = $(_this).data('id');
            $.post('/admin/modules/show', dataPost).done(function (response) {
                ...
                $('#showDetail_Modal').modal('toggle');
                window.urlMethod = '/admin/modules/update/' + $(_this).data('id');
            });
        }
